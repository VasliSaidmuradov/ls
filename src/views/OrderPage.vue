<template>
  <div class="order-page layout">
    <div class="order-page__header">
      ...
      <MainBtn
        class="order-page__header-unfurlText"
    ...
    <div class="order-page__documents">
      <div class="order-page__documents-upload">
        <h6 class="order-page__documents-title">Прикрепить документ</h6>
...
export default class OrderPage extends AnalyzesMixin {
  ...
  $refs: IRefs & Vue['$refs'];
  created() {
    this.$store.dispatch('orders/getOrderedService', this.$route.params.id);
  ...
  get orderedService(): IDashBoard.ILatestResult {
  ...
  getCurrentOrderStatus(statusId: number) {
  ...
  toggleFileModal(val: boolean) {
    this.$store.commit('documents/setPropertyInStore', { name: 'addDocumentToOrder', value: val });
